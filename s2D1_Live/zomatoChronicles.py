...
def newOrder():
    ...
    #  status=request.json['status']
     ...
     print(order.oId)
     return jsonify({'message':'Order receved'})
...
def updateOrder(oId):
     ...
     for i in ordersList:
          if i.oId==oId:
               i.status=status
               return jsonify({'orderid':oId,'Customer name':i.cName,'status':status,'message':'order status Change'})
